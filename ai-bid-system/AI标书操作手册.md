...
使用前必读：
  使用人员请输入以下链接使用：(http://192.168.169.107:5173/)，建议使用浏览器chrome登录
...
2. **进入前端目录**(如遇一键启动脚本失效)
   - 在 VSCode左侧资源管理器或文件夹中，展开项目目录
   - 点击后端windows启动脚本和前端端windows启动脚本
...
5. **访问系统**
   - 打开浏览器访问：http://localhost:5173(本机服务器)/使用人员请打开(http://192.168.169.107:5173/)
